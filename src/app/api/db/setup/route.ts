import { ensureSchema, seedAllProducts, getProducts } from "@/lib/db"

// One-time (idempotent) database initialisation. Creates the products table and
// seeds every security solution's products from the static data file. Safe to
...
async function run() {
  try {
    await ensureSchema()
    const seeded = await seedAllProducts()
